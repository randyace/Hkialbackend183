...
// ── Historical guest registry (all guests who ever visited the lounge) ───────
...
// ── Explicit booking overrides — ensures the detail page matches the list ─────
const EXPLICIT_BOOKING_OVERRIDES: Record<number, Partial<Booking>> = {
  55: {
...
// ── Merge dialog: fields that are permanently fixed (never mergeable) ─────────
const MERGE_FIXED_FIELDS = new Set([
  'Full Name',
  'Gender',
  'Date of Birth',
  'Passport No.',
...
const isPassengerFilled = (p: PassengerDetail): boolean =>
  !!(p.title || p.firstName || p.lastName || p.travelDocNo || p.membershipNo || p.ageGroup || p.birthdayDay);

const buildInitialPassengers = (total: number, bookingId: number): PassengerDetail[] =>
  Array.from({ length: total }, (_, idx) => {
    const seed = mockPassengerSeeds[(bookingId + idx) % mockPassengerSeeds.length];
    return { title: seed.title, firstName: seed.firstName, lastName: seed.lastName,
...
const emptyNonFlyingGuest = (): NonFlyingGuest => ({
...
const buildInitialNonFlyingGuests = (total: number, bookingId: number): NonFlyingGuest[] =>
  Array.from({ length: total }, (_, idx) => {
...
const generateMockBooking = (id: number): Booking => {
  ...
  // Apply explicit overrides for sample bookings added to the list page
